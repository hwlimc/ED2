...
The builder writes `met/ED_MET_DRIVER_HEADER`, monthly met HDF5 files, optional `ed_inputs/` files, optional prescribed phenology text files, optional ED2.2 `.sss/.pss/.css` initial files, `fragments/*.nml` snippets for an ED2IN copy, and `MANIFEST.txt`. Filled examples and a unit dictionary are in `projects/data/staging_examples/`.
