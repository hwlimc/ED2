...
exec "${container_cmd}" exec \
   ...
   "${ED2_SIF}" \
   bash -lc "cd '${ED2_RUN_DIR}' && ulimit -s unlimited && export OMP_NUM_THREADS='${OMP_NUM_THREADS}' I_MPI_PIN='${I_MPI_PIN}' && exec mpirun -np '${ED2_MPI_RANKS}' ed2 -f '${ED2_INPUT}'"
